...
void createModelTextureImage(const char *imagePath)
{
    ...
    createImage((uint32_t) texWidth, (uint32_t) texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
        ...
        &vkc.modelTexture.image, &vkc.modelTexture.deviceMemory);

    transitionImageLayout(vkc.modelTexture.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    copyBufferToImage(stagingBuffer, vkc.modelTexture.image, (uint32_t) texWidth, (uint32_t) texHeight);
    transitionImageLayout(vkc.modelTexture.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
void createTextureSamplers()
{
    ...
    if (vkCreateSampler(vkc.device, &samplerInfo, NULL, &vkc.modelTexture.sampler) != VK_SUCCESS) {
        SDL_Log("failed to create texture sampler for model texture!\n");
    }
    if (vkCreateSampler(vkc.device, &samplerInfo, NULL, &vkc.offscreenTexture.sampler) != VK_SUCCESS) {
        SDL_Log("failed to create texture sampler for offscreen texture!\n");
    }
}
...
void createDescriptorSets()
{
    ...
    if (vkAllocateDescriptorSets(vkc.device, &allocInfo, &vkc.descriptorSets.shadedOffscreen) != VK_SUCCESS) {
        SDL_Log("failed to allocate descriptor set (offscreen)!");
        return;
    }

    {
        ...
        imageInfo.imageView = vkc.modelTexture.imageView;
        imageInfo.sampler = vkc.modelTexture.sampler;
        ...
        descriptorWrites[0].dstSet = vkc.descriptorSets.shadedOffscreen;
        ...
        descriptorWrites[1].dstSet = vkc.descriptorSets.shadedOffscreen;
...

    {
        ...
        imageInfo.sampler = vkc.offscreenTexture.sampler;
...
void initVulkan(const char *texturePath)
{
    createDepthResources();
    createModelTextureImage(texturePath);
    createOffscreenTextureImage();
...
void cleanupVulkan()
{
    ...
    vkDestroySampler(vkc.device, vkc.modelTexture.sampler, NULL);
    vkDestroyImageView(vkc.device, vkc.modelTexture.imageView, NULL);
    vkDestroyImage(vkc.device, vkc.modelTexture.image, NULL);
    vkFreeMemory(vkc.device, vkc.modelTexture.deviceMemory, NULL);
    vkDestroySampler(vkc.device, vkc.offscreenTexture.sampler, NULL);
